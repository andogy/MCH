...
public class ConfigMain {
   ...
   public static Logger recode_logger = new Logger("Config Thread");
    ...
    public static String excessProcess = "excessProcess@Disable";
    ...
    public static void uploadConfig() {
        for(String s : Arrays.asList(ConfigMain.colorSet, ConfigMain.languageSet, ConfigMain.exButtonSet, ConfigMain.fastLoadSet, ConfigMain.onTopSet, ConfigMain.saveCache, ConfigMain.saveErrorLog, ConfigMain.saveRunLog, ConfigMain.autoPUDSet, ConfigMain.saveHistorySet, ConfigMain.input_command, ConfigMain.minecraftListenFlushSpeedLevel, ConfigMain.showInvalidCommand, ConfigMain.showCommands, ConfigMain.priorityDisplay, ConfigMain.toWiki,ConfigMain.excessProcess)) {
    ...
    public static void Reads(String s) {
        ...
        {
            int excess = s.indexOf("excessprocess@enable");
            int noExcess = s.indexOf("excessprocess@disable");

            if(! (excess != - 1 & noExcess != - 1)) {
                if(excess != - 1) {
                    Community.excessProcess = true;
                    excessProcess = "excessProcess@enable";
                    s = "";
                } else if(noExcess != - 1) {
                    Community.excessProcess = false;
                    excessProcess = "excessProcess@disable";
                    s = "";
                }
            }
        }
